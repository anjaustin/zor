// =============================================================================
// 384x384x384 = 56,623,104 NODES - ~25.7D Hypercube
...
#define CUBE_SIZE 384
#define NUM_NODES 56623104
#define THREADS 256
#define THRESHOLD 4
...
__global__ void plasticity(Node* n, unsigned long long* res_cnt, unsigned long long* rew_cnt, int cycle) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= NUM_NODES) return;
...
int main() {
    printf("╔═══════════════════════════════════════════════════════════════════╗\n");
    printf("║     384×384×384 = 56,623,104 NODES (~25.7D Hypercube)             ║\n");
    ...
    size_t node_mem = (size_t)NUM_NODES * sizeof(Node);
    size_t state_mem = NUM_NODES;
    printf("Required: %.1f GB nodes + %.1f MB states = %.1f GB total\n\n",
           node_mem / 1e9, state_mem / 1e6, (node_mem + state_mem) / 1e9);

    Node* d_n; uint8_t* d_st; unsigned long long *d_res, *d_rew;
    ...
    cudaMalloc(&d_rew, sizeof(unsigned long long));
    ...
    unsigned long long total_rew = 0;
    int max_cyc = 3000;
    ...
    for (int c = 0; c < max_cyc; c++) {
        ...
        cudaMemset(d_res, 0, 8); cudaMemset(d_rew, 0, 8);
        plasticity<<<blk, THREADS>>>(d_n, d_res, d_rew, c);

        unsigned long long h_res, h_rew;
        cudaMemcpy(&h_res, d_res, 8, cudaMemcpyDeviceToHost);
        cudaMemcpy(&h_rew, d_rew, 8, cudaMemcpyDeviceToHost);
        total_rew += h_rew;

        if (c % 500 == 0 || c < 10 || h_res == (unsigned long long)NUM_NODES)
            printf("%d,%llu,%.1f%%\n", c, h_res, (h_res*100.0)/NUM_NODES);

        if (h_res == (unsigned long long)NUM_NODES) {
            cudaEventRecord(stop); cudaEventSynchronize(stop);
            float ms; cudaEventElapsedTime(&ms, start, stop);
            printf("\n╔═══════════════════════════════════════════════════════════════════╗\n");
            printf("║     56,623,104 NODES CONVERGED at cycle %d                       ║\n", c);
            printf("╚═══════════════════════════════════════════════════════════════════╝\n");
            printf("    Time: %.1f sec | Rewires: %llu\n", ms/1000.0, total_rew);
            printf("    Throughput: %.2f M node-cycles/sec\n",
                   (double)NUM_NODES * c / (ms / 1000.0) / 1e6);
            printf("\nTHE SCALING LAW:\n");
            ...
            printf("  16777216 nodes(24D)  → 1063 cycles\n");
            printf("  56623104 nodes(25.7D)→ %d cycles\n", c);
            break;
        }
    }
    ...
    if (h_res < (unsigned long long)NUM_NODES) {
        cudaEventRecord(stop); cudaEventSynchronize(stop);
        float ms; cudaEventElapsedTime(&ms, start, stop);
        printf("\nReached %llu/%d (%.1f%%) after %d cycles\n",
               h_res, NUM_NODES, (h_res*100.0)/NUM_NODES, max_cyc);
        printf("Time: %.1f sec | Total rewires: %llu\n", ms/1000.0, total_rew);
    }
