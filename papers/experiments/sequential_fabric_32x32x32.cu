// 32x32x32 = 32,768 nodes - 15D Hypercube
...
__global__ void init_fabric(Node* n) {
    ...
    n[i].lfsr = 0xBEEF ^ (i * 0x1337) ^ (i << 8);
    for (int d = 0; d < 6; d++) n[i].neighbors[d] = calc_nb(i, d);
}

int main() {
    printf("╔═══════════════════════════════════════════════════════════════════╗\n");
    printf("║     32×32×32 = 32,768 NODES (15D Hypercube)                       ║\n");
    printf("╚═══════════════════════════════════════════════════════════════════╝\n\n");
    ...
    for (int c = 0; c < max_cyc; c++) {
        ...
        if (c % 25 == 0 || h_res == NUM_NODES)
            printf("%d,%d,%d%%,%d\n", c, h_res, (h_res*100)/NUM_NODES, total_rew);

        if (h_res == NUM_NODES) {
            cudaEventRecord(stop); cudaEventSynchronize(stop);
            float ms; cudaEventElapsedTime(&ms, start, stop);
            printf("\n*** CONVERGED at cycle %d ***\n", c);
            printf("    32,768/32,768 nodes (100%%)\n");
            printf("    Rewires: %d\n", total_rew);
            printf("    Time: %.2f ms (%.0f cycles/sec)\n\n", ms, c/(ms/1000));
            printf("Scaling: 4x4x4→8x8x8→16x16x16→32x32x32\n");
            printf("         64 → 512 → 4096 → 32768 nodes\n");
            printf("         158 → 113 → 202 → %d cycles\n", c);
            break;
        }
    }
