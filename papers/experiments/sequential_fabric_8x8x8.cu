// =============================================================================
// SEQUENTIAL_FABRIC_8x8x8.cu - Scaled Sequential Swap Implementation
// =============================================================================
//
// 8x8x8 = 512 nodes with sequential phase processing
// Same algorithm that achieved 100% on 4x4x4, now at 8x scale
//
// =============================================================================
...
// =============================================================================
// SEQUENTIAL PHASE KERNEL - Uses global memory for large fabric
// =============================================================================

__global__ void sequential_phase_8x8x8(
    SeqNode* nodes,
    uint8_t* all_states,  // Global array of all states
    int phase
) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= NUM_NODES) return;

    // Get neighbor value for THIS phase from global state array
...
// Update global state array from nodes
...
__global__ void reset_resonance_8x8x8(SeqNode* nodes) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= NUM_NODES) return;
...
// Plasticity update
__global__ void plasticity_update_8x8x8(
    SeqNode* nodes,
    int* resonant_count,
    int* rewire_count
) {
...
// Initialization
__global__ void init_seq_fabric_8x8x8(SeqNode* nodes) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= NUM_NODES) return;
...
int main() {
    printf("╔═══════════════════════════════════════════════════════════════════╗\n");
    printf("║     SEQUENTIAL FABRIC 8x8x8: 512 Nodes                           ║\n");
    printf("║     Same algorithm that achieved 100%% on 4x4x4                   ║\n");
    printf("╚═══════════════════════════════════════════════════════════════════╝\n\n");
    ...
    init_seq_fabric_8x8x8<<<blocks, THREADS_PER_BLOCK>>>(d_nodes);
    ...
    printf("Architecture:\n");
    printf("  8x8x8 torus (512 nodes)\n");
    printf("  6 sequential phases per cycle\n");
    printf("  Comparator swap (Verilog-style)\n");
    ...
    for (int cycle = 0; cycle < max_cycles; cycle++) {
        // Reset resonance
        reset_resonance_8x8x8<<<blocks, THREADS_PER_BLOCK>>>(d_nodes);

        // Run 6 sequential phases
        for (int phase = 0; phase < NUM_NEIGHBORS; phase++) {
            sequential_phase_8x8x8<<<blocks, THREADS_PER_BLOCK>>>(d_nodes, d_states, phase);
            cudaDeviceSynchronize();
            // Update state array after each phase
            sync_states<<<blocks, THREADS_PER_BLOCK>>>(d_nodes, d_states);
            cudaDeviceSynchronize();
        }
        ...
        plasticity_update_8x8x8<<<blocks, THREADS_PER_BLOCK>>>(d_nodes, d_resonant, d_rewires);
        ...
        if (h_res == NUM_NODES) {
            ...
            printf("    512/512 nodes resonant (100%%)\n");
            printf("    Total rewires: %d\n", total_rewires);
            printf("    Time: %.2f ms (%.1f cycles/sec)\n", ms, cycle / (ms / 1000.0));
            printf("\n");
            printf("═══════════════════════════════════════════════════════════════════\n");
            printf("     FUNKY CONVERGENCE AT SCALE!\n");
            printf("═══════════════════════════════════════════════════════════════════\n");
            printf("\n");
            printf("Scaling comparison:\n");
            printf("  4x4x4:   64 nodes, ~158 cycles\n");
            printf("  8x8x8:  512 nodes, %d cycles\n", cycle);
            printf("  Factor: 8x nodes, %.1fx cycles\n\n",
                   (float)cycle / 158.0f);
            break;
        }
    }
