...
#define THREADS_PER_SUPERCLUSTER 16

struct Cluster { uint64_t data[4][8]; };
struct FabricLayer { Cluster clusters[4]; };
struct SuperCluster { FabricLayer layers[4]; };
...
__device__ __forceinline__ uint64_t geometry_add(uint64_t a, uint64_t b) {
    ...
    int G1 = g1 | (p1 & g0);
    g2 = g2 | (p2 & g1) | ((p2 & p1) & g0);
    g3 = g3 | (p3 & g2) | ((p3 & p2) & G1);
    g4 = g4 | (p4 & g3) | ((p4 & p3) & g2) | ((p4 & p3 & p2 & p1) & g0);
    g5 = g5 | (p5 & g4) | ((p5 & p4) & g3) | ((p5 & p4 & p3 & p2) & G1);
    g6 = g6 | (p6 & g5) | ((p6 & p5) & g4) | ((p6 & p5 & p4 & p3) & g2);
    g7 = g7 | (p7 & g6) | ((p7 & p6) & g5) | ((p7 & p6 & p5 & p4) & g3);
...
__global__ void geometry_step(SuperCluster* sc, int num_sc, int steps) {
    ...
    Cluster* c = &sc[sc_id].layers[tid / 4].clusters[tid % 4];
    uint64_t d[4][8];
...
int main() {
    printf("MDS GEOMETRY - CPU TIMING\n");
    printf("=========================\n\n");
    ...
    long long adds_per_iter = (long long)num_sc * THREADS_PER_SUPERCLUSTER * 4 * 8 * steps;

    printf("SuperClusters: %d\n", num_sc);
    printf("Steps/kernel: %d\n", steps);
    printf("Iterations: %d\n", iters);
    printf("Memory: %.2f MB\n\n", size / 1e6);
    ...
    long long total = adds_per_iter * iters;
    double throughput = total / (ms / 1000.0);
