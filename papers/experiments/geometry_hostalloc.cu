...
int main() {
    printf("CUDA with cudaHostAlloc (mapped pinned memory)\n\n");
    ...
    cudaError_t err = cudaHostAlloc(&h_data, size, cudaHostAllocMapped);
    printf("cudaHostAlloc: %s (ptr=%p)\n", cudaGetErrorString(err), h_data);
    ...
    printf("cudaHostGetDevicePointer: %s (ptr=%p)\n", cudaGetErrorString(err), d_data);
    ...
    printf("First result: %lu (expected change from %d)\n", h_data[0].v[0], 0);
