...
int main() {
    printf("MDS GEOMETRY - SCALED BENCHMARK\n");
    printf("================================\n\n");

    int num_sc = 65536;  // 16x more
    int steps = 10000;   // 10x more
    int iters = 100;     // 10x more
    
    size_t size = sizeof(SuperCluster) * num_sc;
    long long adds_per_step = (long long)num_sc * THREADS_PER_SUPERCLUSTER * 4 * 8;

    printf("SuperClusters: %d\n", num_sc);
    printf("64-bit adds/step: %lld\n", adds_per_step);
    ...
    long long total = adds_per_step * steps * iters;
    double throughput = total / (ms / 1000.0);

    printf("Time: %.2f ms\n", ms);
    printf("Total adds: %.2f trillion\n", total / 1e12);
    printf("Throughput: %.2f trillion/sec\n", throughput / 1e12);
    printf("ns/add: %.6f\n", (ms * 1e6) / total);
